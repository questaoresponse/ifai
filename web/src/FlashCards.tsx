...
const GEMINI_API_KEY = "AIzaSyBRTlHd_tDfYYBcyZ8jL0ptc3aGbpahW4E";
...
function FlashCards(){
    ...
    const flashcardApp: flashcardAppInterface = {
    ...
    const generateFlashCard = async () => {
        ...
        try {
            ...
            flashcardApp.saveDecks(newDeck);
            setDecks(newDeck)
        } catch (error) {
    ...
    return <>
        <div id="flash" className="container page">
            <main>
                ...
                <div className="card">
                    <h2>Meus decks</h2>
                    <button id="createDeckBtn" className="btn" onClick={()=>flashcardApp.toggleDeckForm(true)}>
                    ...
                    <div>
                        <button id="saveDeckBtn" className="btn btn-success" onClick={()=>flashcardApp.saveNewDeck()}>Criar</button>
                        <button id="cancelDeckBtn" className="btn btn-outline" onClick={()=>{flashcardApp.toggleDeckForm(false);flashcardApp.resetDeckForm();}}>
                        Cancelar
                        </button>
                    </div>
                    </div>
                    ...
                    <div id="decksList" className="decks-container">
                        { isFirstDeck ? <div className="empty-state">
                            <i className="fas fa-inbox"></i>
                            <h3>Nenhum deck encontrado</h3>
                            <p>Crie seu primeiro deck para começar a adicionar flashcards</p>
                            <button className="btn" onClick={()=>flashcardApp.toggleDeckForm(true)}>Criar deck</button>
                        </div> : decks.map((deck, index: number)=>{
                            return <div className="deck" onClick={()=>flashcardApp.showDeck(deck.id)} key={index}>
                                <div className="deck-actions">
                                <button onClick={()=>flashcardApp.deleteDeck(deck.id)}>
                ...
                <div className="card">
                    ...
                    <div id="flash-btns">
                        ...
                        <button id="addFlashcardBtn" className="btn" onClick={()=>flashcardApp.toggleFlashcardForm(true)}>
                            <i className="fas fa-plus"></i> Adicionar flashcard
                        </button>
                        <button id="studyDeckBtn" className="btn btn-success" onClick={()=>flashcardApp.startStudyMode()}>
                            <i className="fas fa-graduation-cap"></i> Estudar deck
                        </button>
                        <button id="generateFlashCard" onClick={()=>setFlashCardMenu(true)}>Gerar flashcard</button>
                    </div>
                    ...
                    <div>
                        <button id="saveFlashcardBtn" className="btn btn-success" onClick={()=>flashcardApp.saveNewFlashcard()}>
                        Salvar flashcard
                        </button>
                        <button id="cancelFlashcardBtn" className="btn btn-outline" onClick={()=>{flashcardApp.toggleFlashcardForm(false);flashcardApp.resetFlashcardForm();}}>
                        Cancelar
                        </button>
                    </div>
                    </div>
                    ...
                    <div id="flashcardsList" className="flashcards-container">{currentDeck == "" ?    
                        <div className="empty-state">
                            ...
                            <button className="btn" onClick={()=>flashcardApp.toggleFlashcardForm(true)}>Adicionar flashcard</button>
                        </div>
                    : decks.filter(deck=>deck.id == currentDeck)[0].flashcards.map((card, index: number)=>{
                        return <div className="flashcard" onClick={(e: any)=>!e.target.closest(".card-actions") && e.target.classList.toggle("show-back")} key={index}>
                            <div className="flashcard-front">
                                ...
                                <button className="btn btn-danger" onClick={()=>flashcardApp.deleteFlashcard(card.id)}>
                                    <i className="fas fa-trash"></i>
                                </button>
                            </div>
                        </div>
                ...
                <div className="card study-mode">
                    ...
                    <div className="study-card" id="studyCard" onClick={()=>flashcardApp.toggleStudyCard()}>
                    ...
                    <button id="prevCardBtn" className="btn btn-outline" onClick={()=>flashcardApp.showPreviousCard(currentCardIndex)}>
                        <i className="fas fa-arrow-left"></i> Anterior
                    </button>
                    <button id="nextCardBtn" className="btn" onClick={()=>flashcardApp.showNextCard(currentCardIndex)}>
